...
`git log`를 확인하여 squad가 수정한 파일 중 이후 되돌려진(reverted) 파일을 감지합니다:
- fix-history.jsonl의 파일 목록과 `git log --diff-filter=M` 대조
- `git diff`로 **동일 파일의 동일 라인 범위**가 사용자 커밋에서 변경된 경우에만 잠재적 리버트로 표시
- 단순히 같은 파일이 수정된 것은 리버트로 간주하지 않음 (다른 영역 수정은 정상)
